...
class HomeResponsive_Page():  
     ...
     def openUrl(self):
      ...
      locator = "modelText";  
      self.driver.find_element_by_id(locator).send_keys(reader.readInI('test', "make2"))
    ...
    #Click Search button at home page
     ...
     def clickOnEquipPrivateParty(self):
        reader = ReaderFile.ReadXML()
        locator = reader.readXml('EquipPrivateParty')
        time.sleep(3)
        self.driver.find_element_by_xpath(locator).click()
        
     def getprivatepartydetails(self):
        reader = ReaderFile.ReadXML()
        locator = reader.readXml('EquipPrivatePartyDetails')
        time.sleep(3)
